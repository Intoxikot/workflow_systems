...
/* Внешние ключи отдельно (уже включены в таблицы)
ALTER TABLE store.income 
ADD CONSTRAINT income_fk0
FOREIGN KEY (product_id) 
REFERENCES store.product (product_id);

ALTER TABLE store.income 
ADD CONSTRAINT income_fk1 
FOREIGN KEY (document_id) 
REFERENCES store.document (document_id);

ALTER TABLE store.outcome 
ADD CONSTRAINT outcome_fk0
FOREIGN KEY (product_id) 
REFERENCES store.product (product_id);

ALTER TABLE store.outcome 
ADD CONSTRAINT outcome_fk1
FOREIGN KEY (document_id) 
REFERENCES store.document (document_id);

ALTER TABLE store.inventory_item 
ADD CONSTRAINT inventory_item_fk0 
FOREIGN KEY (document_id) 
REFERENCES store.document (document_id);

ALTER TABLE store.inventory_item  
ADD CONSTRAINT inventory_item_fk1
FOREIGN KEY (product_id) 
REFERENCES storeproduct (product_id);
*/